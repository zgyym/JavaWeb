...
2. Servlet的继承关系 - 重点查看的是服务方法（service()）
    ...
    3) 服务方法： 当有请求过来时，service方法会自动响应（其实是tomcat容器调用的）
            在HttpServlet类中我们会去分析请求的方式：到底是get、post、head还是delete等等
            然后再决定调用的是哪个do开头的方法
...
3. Servlet的生命周期
    ...
    5） Servlet在容器中是：单例的、线程不安全的
        ...
        - 我们已经知道了servlet是线程不安全的，给我们的启发是： 尽量的不要在servlet中定义成员变量。如果不得不定义成员变量，那么：①不要去修改成员变量的值 ②不要去根据成员变量的值做一些逻辑判断
...
5. 会话
    ...
    2） 会话跟踪技术
        ...
        - 同一个客户端在一段时间内是一个session，而不是多少个网页多少个session，同一个浏览器中不同的请求用的是同一个session
        - session区分的是客户端（浏览器），而不是同一个客户端访问的不同地址
        - 常用的API：
